...
if "%OLLAMA_LAUNCHER_APP_ROOT%"=="" (
    set "APP_ROOT=%~dp0..\.."
) else (
    set "APP_ROOT=%OLLAMA_LAUNCHER_APP_ROOT%"
)
set "APPDATA_OLLAMA=%OLLAMA_LAUNCHER_CONFIG_DIR%"
if "%APPDATA_OLLAMA%"=="" set "APPDATA_OLLAMA=%APPDATA%\ollamaLauncher"
set "CACHE_OLLAMA=%OLLAMA_LAUNCHER_CACHE_DIR%"
if "%CACHE_OLLAMA%"=="" if "%LOCALAPPDATA%"=="" set "CACHE_OLLAMA=%TEMP%\ollamaLauncher\Cache"
if "%CACHE_OLLAMA%"=="" set "CACHE_OLLAMA=%LOCALAPPDATA%\ollamaLauncher\Cache"
set "FETCH_MODELS_SCRIPT=%OLLAMA_LAUNCHER_FETCH_SCRIPT%"
if "%FETCH_MODELS_SCRIPT%"=="" set "FETCH_MODELS_SCRIPT=%APP_ROOT%\fetch_models.ps1"
if not exist "%FETCH_MODELS_SCRIPT%" set "FETCH_MODELS_SCRIPT=%APPDATA_OLLAMA%\fetch_models.ps1"
set "SELECTOR_SCRIPT=%OLLAMA_LAUNCHER_MODEL_SELECTOR_SCRIPT%"
if "%SELECTOR_SCRIPT%"=="" set "SELECTOR_SCRIPT=%APPDATA_OLLAMA%\model_selector.ps1"
set "CONTEXT_SELECTOR_SCRIPT=%OLLAMA_LAUNCHER_CONTEXT_SELECTOR_SCRIPT%"
if "%CONTEXT_SELECTOR_SCRIPT%"=="" set "CONTEXT_SELECTOR_SCRIPT=%APPDATA_OLLAMA%\context_selector.ps1"
set "LOCAL_SELECTOR_SCRIPT=%OLLAMA_LAUNCHER_LOCAL_SELECTOR_SCRIPT%"
if "%LOCAL_SELECTOR_SCRIPT%"=="" set "LOCAL_SELECTOR_SCRIPT=%APPDATA_OLLAMA%\local_selector.ps1"
...
set "REPOS_CONFIG=%OLLAMA_LAUNCHER_REPOS_CONFIG%"
if "%REPOS_CONFIG%"=="" set "REPOS_CONFIG=%APPDATA_OLLAMA%\repos.json"
...
REM PowerShell entrypoint owns setup in normal launches. Keep the legacy
REM setup labels for direct script invocation during the transition.
if not "%OLLAMA_LAUNCHER_SETUP_DONE%"=="1" (
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%APP_ROOT%\ollama_wrapper.ps1" -Pull -ModelName "!pull_target!"
if %errorlevel% neq 0 (
    echo.
...
echo.
powershell -NoProfile -ExecutionPolicy Bypass -File "%APP_ROOT%\ollama_wrapper.ps1" -Run -ModelName "!run_model!" -ContextLength !CONTEXT_LENGTH!
if %errorlevel% neq 0 (
    echo.
...
if exist "%APP_ROOT%\fetch_models.ps1" (
    set "FETCH_MODELS_SCRIPT=%APP_ROOT%\fetch_models.ps1"
    copy /Y "%APP_ROOT%\fetch_models.ps1" "%APPDATA_OLLAMA%\fetch_models.ps1" >nul 2>&1
